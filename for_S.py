...
if uploaded_file:
 ...
 # 批量生成所有比值（你的代码没问题）
    ...
    # 5. 美化表头（关键）
    pivot_out = pivot_out.rename(columns=rename_col)

    st.success("处理完毕，结果宽表预览：")
    st.dataframe(pivot_out)    # 展示已经美化的表

    # 提供下载按钮（也要导出美化后的pivot_out）
    output = io.BytesIO()
    
    st.write(type(pivot_out))
    st.write(pivot_out.head())
    st.write(pivot_out.columns)

    pivot_out.to_excel(output, index=False, engine='openpyxl')
    output.seek(0)
    st.download_button(
        label="下载Excel结果",
